...
path = 'crawled_img/' + '_' + str(maxImages)
...
for i in range(1, int(pages) + 1):
    ...
    # driver.get('https://www.istockphoto.com/kr/사진/' + NAME + '?phrase=' + keyword)
    # driver.get('https://www.istockphoto.com/kr/search/2/image?alloweduse=availableforalluses&phrase=' + keyword + '&page=' + str(i))
    driver.get('https://www.shutterstock.com/ko/search/kids-full-body?page=' + str(i))
    ...
    for img in picture:
        ...
        if len(srcset):
            ...
            new_filename = str(cnt) + ".jpg"

            # 저장 경로 결정
            print(new_filename)
            print("파일명")

            saveUrl = path + '/' + new_filename
            print(saveUrl)

            # 파일 저장
            # user-agent 헤더를 가지고 있어야 접근 허용하는 사이트도 있을 수 있음(pixabay가 이에 해당)
            req = urllib.request.Request(src, headers={'User-Agent': 'Mozilla/5.0' | 'Chrome/89.0.4389.82'})
